...
export function TemplateStrip() {
  ...
  return (
    <section className="overflow-hidden py-20 sm:py-28" aria-roledescription="carousel" aria-label="תבניות חותמות">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            <button
              ...
              aria-label="הקודם"
              className="grid h-11 w-11 place-items-center rounded-full border border-line bg-white shadow-soft transition hover:border-blue hover:text-blue disabled:opacity-35"
            >
            ...
            <button
              ...
              onClick={() => go(1)}
              aria-label="הבא"
              className="grid h-11 w-11 place-items-center rounded-full border border-line bg-white shadow-soft transition hover:border-blue hover:text-blue"
            >
        ...
        <div className="-mx-4 mt-6 flex gap-2 overflow-x-auto px-4 pb-1 [scrollbar-width:none] sm:mx-0 sm:px-0" role="tablist" aria-label="סינון לפי תחום">
          {[{ id: 'all' as const, label: 'הכול' }, ...CATS].map((c) => (
            <button
              ...
              onClick={() => setCat(c.id)}
              className={`shrink-0 rounded-full px-4 py-2 text-sm font-medium transition ${cat === c.id ? 'bg-ink text-white shadow-soft' : 'border border-line bg-white text-ink-2 hover:border-ink/30'}`}
            >
              {c.label}
            </button>
          ))}
      ...
      {/* Contained slider: same width as the heading, not edge-to-edge. */}
      <div className="container-x">
        <div className="relative mt-6">
          {/* Edge fades */}
          <div aria-hidden className={`pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-gradient-to-l from-white transition-opacity sm:w-16 ${edges.start ? 'opacity-0' : 'opacity-100'}`} />
          <div aria-hidden className={`pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-gradient-to-r from-white transition-opacity sm:w-16 ${edges.end ? 'opacity-0' : 'opacity-100'}`} />
          <div
            ref={track}
            onScroll={measure}
            ...
            onFocus={pause(true)}
            onBlur={pause(false)}
            className="flex snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain rounded-3xl pt-2 pb-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          >
            {list.map((t, i) => {
              ...
              return (
                ...
                >
                  <div className="relative grid aspect-[4/3] place-items-center bg-gradient-to-b from-surface to-blue-50/60 p-6">
                    ...
                    <span className="absolute top-3 left-3 rounded-full bg-ink/80 px-2 py-1 text-[10px] font-medium text-white tabular-nums">{String(i + 1).padStart(2, '0')}</span>
